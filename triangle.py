...
class TriangleApplication(Application):
    ...
    def create_triangle(self):
        ...
        (triBuffer,triMem) = self.createBuffer(
            vertices_size,
            vk.BUFFER_USAGE_VERTEX_BUFFER_BIT | vk.BUFFER_USAGE_TRANSFER_DST_BIT,
            vk.MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
        self.copyBuffer(self.stagingBuffer, triBuffer, 0, vertices_size)
        ...
        self.triangle['buffer'] = triBuffer
        self.triangle['memory'] = triMem
        self.triangle['indices_buffer'] = indiceBuffer
        self.triangle['indices_memory'] = indiceMem
        self.describe_bindings()

    def create_uniform_buffers(self):
        (self.uniform_data['buffer'], self.uniform_data['memory']) = self.createBuffer(
            sizeof(Mat4)*3,
